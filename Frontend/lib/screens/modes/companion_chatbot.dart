...
class _CompanionChatBotState extends State<CompanionChatBot> {
  ...
  Future<void> _initializeData() async {
    try {
      ...
      _threadId = await _apiService.getOrCreateThreadId('companion');
      await _loadMessages();
    } catch (e) {
  ...
  Future<void> _initializeData2() async {
    try {
      await _fetchUserData();
      therapistThreadId = await _apiService.getOrCreateThreadId('therapist');
      await _loadMessages();
    } catch (e) {
      print('Error initializing data: $e');
    }
  ...
  void _sendMessage(types.PartialText message) async {
    ...
    if (userInput.isNotEmpty) {
      ...
      try {
        final response =
            await _apiService.getChatbotResponseCompanion(userInput);
              ...
              Navigator.push(
                ...
                MaterialPageRoute(
                  builder: (context) => ChatHistoryPage(
                    therapistThreadId: therapistThreadId,
